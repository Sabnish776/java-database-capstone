...
//      - It uses a LEFT JOIN to fetch the doctor’s available times along with the appointments.
    @Query("""
            ...
            """)
    List<Appointment> findByDoctor_IdAndAppointmentTimeBetween(@Param("doctorId") Long doctorId,
...
//      - Parameters: String doctorName, Long patientId
    @Query("""
            select distinct a
            from Appointment a where lower(a.doctor.name) LIKE lower(concat('%',:doctorName,'%')) 
...
//      - Parameters: String doctorName, Long patientId, int status
    @Query("""
            select distinct a from Appointment a where lower(a.doctor.name) LIKE lower(concat('%',:doctorName,'%'))
